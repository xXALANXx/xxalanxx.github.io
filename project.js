...
(function () {
  ...
  article.className = "project-article" + (images.length > 0 ? " project-article--split" : "");
  const dateLine = p.date ? `<p class="project-date">${escapeHtml(p.date)}</p>` : "";
  /**
   * Long description: escaped text with optional Stage formatting, or trusted HTML when `useHtmlLongDesc` is set.
   * @type {string}
   */
  const longDescHtml = p.longDesc
    ? p.useHtmlLongDesc
      ? `<div class="long-desc long-desc--rich">${p.longDesc}</div>`
      : `<div class="long-desc">${formatLongDesc(p.longDesc)}</div>`
    : "";
  ...
  const heroBlock =
    images.length > 0
      ? ""
      : `<div class="project-hero-image${heroClass}">${heroImg}</div>`;
  ...
  article.innerHTML =
    ...
    <div class="project-detail-body">
      ...
      <aside class="project-detail-aside" aria-label="Project gallery">
        <h3 class="project-gallery-heading">Gallery</h3>
        <div class="project-gallery project-gallery--rail${galleryClass}">${galleryInner}</div>
      </aside>
    </div>`
      : `${heroBlock}
    ${headerBlock}
